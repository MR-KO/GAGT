...
cell c, int v0, int v1, int v2, int v3) {
	...
	if (num_greater == 3 || num_lesser == 3) {
		...
		if (pattern == 0x0001 || pattern == 0x1110) {
			...
			triangles->p[0] = interpolate_points(isovalue, c.p[v1], c.p[v0],
				c.value[v1], c.value[v0]);
			...
			triangles->p[0] = interpolate_points(isovalue, c.p[v2], c.p[v1],
				c.value[v2], c.value[v1]);
			triangles->p[1] = interpolate_points(isovalue, c.p[v2], c.p[v3],
				c.value[v2], c.value[v3]);
			triangles->p[2] = interpolate_points(isovalue, c.p[v2], c.p[v0],
				c.value[v2], c.value[v0]);
		} else if (pattern == 0x1000 || pattern == 0x0111) {
			triangles->p[0] = interpolate_points(isovalue, c.p[v3], c.p[v2],
				c.value[v3], c.value[v2]);
			triangles->p[1] = interpolate_points(isovalue, c.p[v3], c.p[v0],
				c.value[v3], c.value[v0]);
			triangles->p[2] = interpolate_points(isovalue, c.p[v3], c.p[v1],
				c.value[v3], c.value[v1]);
		} else {
